...
func _process(delta):
	...
	var scaled_look_ahead: float = lerpf(look_ahead_distance, look_ahead_distance * 0.2, zoom_t)
	...
	# --- Y CAMERA SHIFT BASED ON ZOOM ---
	zoom_t = clamp(zoom_t, 0.0, 1.0)
	
	position.y = lerp(min_y_offset, max_y_offset, zoom_t)
